...
import {db} from "#db";
import {broadcast_pack} from "#lib/network";
 ...
 */
export default async (bin: Uint8Array, relay: boolean = false, debug = false): RET_TYPE=>{
    const pack: Pack = Pack.from_binary(bin);
    if (await is_pack_known(<string>pack.r_hash))
        throw new Error("Duplicate pack");
    try {
        await mtx.runExclusive(async (): Promise<void>=>{
            db.batch();
            ...
            if (!await are_all_parents_known(pack))
            ...
            const pre_state: { stabilizers: Map<string, bigint>, stabilization_chain: Array<Pack> } = {
                stabilizers: await db.get_stabilizers(),
