...
describe('[VM] AND', ()=>{
    it('should AND the buffers', async function () {
        const code: Uint8Array = new Uint8Array([OPS.LABEL, OPS.PUSH2, ...bigint2word(12n), ...bigint2word(30n), OPS.AND]);
        const vm: Vm = new Vm(code, 'f', 256, []);

        const {ok, err}: Option<Uint8Array, number> = await vm.execute();
        assert.isUndefined(err, 'No error was produced');
        assert.lengthOf(ok, BALANCE_WIDTH_BYTES);
        assert.strictEqual(binary2bigint(ok), 12n, 'AND produced the expected result');
