...
describe('[VM] INC', ()=>{
    it("should increment the stack's head by one", async function () {
        const code: Uint8Array = new Uint8Array([OPS.LABEL, OPS.PUSH, ...bigint2word(3n), OPS.INC]);
        ...
        assert.strictEqual(binary2bigint(ok), 4n, 'Increment worked');
    ...
    it("should properly handle overflows", async function () {
        const code: Uint8Array = new Uint8Array([OPS.LABEL, OPS.PUSH, ...bigint2word(MAX_INTEGER), OPS.INC]);
        const vm: Vm = new Vm(code, 'f', 256, []);
        const {ok, err}: Option<Uint8Array, EXCEPTIONS> = await vm.execute();
        assert.isUndefined(err, 'No error was produced');
        assert.lengthOf(ok, BALANCE_WIDTH_BYTES, 'We got a buffer as response');
        assert.strictEqual(binary2bigint(ok), 0n, 'Increment worked');
        assert.isUndefined(vm.exception, 'VM produced no exception');
        assert.strictEqual(vm.pc, code.length, 'VM code tape was consumed');
        assert.lengthOf(vm.stack, 0, 'VM stack is empty');
    });
