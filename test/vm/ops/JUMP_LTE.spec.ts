...
describe('[VM] JUMP_LTE', ()=>{
    it("should jump when A < B", async function () {
        const code: Uint8Array = new Uint8Array([OPS.LABEL, OPS.PUSH, ...bigint2word(3n), OPS.PUSH, ...bigint2word(2n), OPS.JUMP_LTE, ...int2qop(BALANCE_WIDTH_BYTES*2+12), 42, 42, 42, 42, OPS.PUSH, ...bigint2word(42n)]);
        ...
        assert.strictEqual(binary2bigint(ok), 42n, 'JUMP_LTE worked');
    ...
    it("should jump when A = B", async function () {
        const code: Uint8Array = new Uint8Array([OPS.LABEL, OPS.PUSH, ...bigint2word(3n), OPS.PUSH, ...bigint2word(3n), OPS.JUMP_LTE, ...int2qop(BALANCE_WIDTH_BYTES*2+12), 42, 42, 42, 42, OPS.PUSH, ...bigint2word(42n)]);
    ...
    it("should throw an exception when jumping to an out of bounds offset", async function () {
        const code: Uint8Array = new Uint8Array([OPS.LABEL, OPS.PUSH, ...bigint2word(3n), OPS.PUSH, ...bigint2word(3n), OPS.JUMP_LTE, ...int2qop(BALANCE_WIDTH_BYTES*3+14), 42, 42, 42, 42, OPS.PUSH, ...bigint2word(42n)]);
        const vm: Vm = new Vm(code, 'f', 256, []);
        const {ok, err}: Option<Uint8Array, EXCEPTIONS> = await vm.execute();
        ...
        assert.strictEqual(vm.pc, BALANCE_WIDTH_BYTES*2+4, 'PC stopped incrementing past JUMP_LTE');
    });
    it("should not jump when A > B", async function () {
        const code: Uint8Array = new Uint8Array([OPS.LABEL, OPS.PUSH, ...bigint2word(3n), OPS.PUSH, ...bigint2word(4n), OPS.JUMP_LTE, ...int2qop(BALANCE_WIDTH_BYTES*2+12), OPS.PUSH, ...bigint2word(3n), OPS.PUSH, ...bigint2word(42n)]);
        const vm: Vm = new Vm(Buffer.from(code), 'f', 256, []);
        const {ok, err}: Option<Uint8Array, EXCEPTIONS> = await vm.execute();
        assert.isUndefined(err, 'No error was produced');
        assert.lengthOf(ok, BALANCE_WIDTH_BYTES);
