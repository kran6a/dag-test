...
import {calculatePriority, convertParamSyntax, isFileIgnored, mergePaths, prioritizeRoutes} from "./utils.js";
...
export const walkTree = (directory: string, tree: string[] = []) => {
    ...
    for (const fileName of readdirSync(directory)) {
        ...
        if (fileStats.isDirectory())
            ...
            results.push({name: fileName, path: directory, rel: mergePaths(...tree, fileName)})
    }
    return results;
}
...
export const generateRoutes = async (files: File[])=>{
    ...
    for (const file of files) {
        ...
        const exports = await import((is_windows ? 'file://' : '')+path.normalize(path.resolve(path.join(file.path, file.name))));
        routes.push({url, priority, exports});
    }
