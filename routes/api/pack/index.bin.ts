...
export const put: Endpoint = async ({request})=>{
    ...
    const {ok, err}: Option<string> = await handle_incoming_pack(pack.binary(), {relay: true}); //This pack was authored by this node. We must relay it to the network
...
export const post: Endpoint = async ({request, url})=>{
    const raw: Uint8Array = new Uint8Array(request.body);
    const {ok, err}: Option<string> = await handle_incoming_pack(raw, {relay: url.searchParams.has('relay')}); //Do not relay this, we assume the author already relayed it. Otherwise, it will be queried when someone references it as a parent.
    if (err)
