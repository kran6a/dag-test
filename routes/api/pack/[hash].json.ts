...
export const get = async ({params}: {params: {hash: string | undefined}})=>{
    ...
    const body: string = toJSON(pack.json());
    return {status: 200, headers: {'cache-control': 'public,immutable', 'content-type':'application/json', 'content-length': body.length}, body};
}
 ...
 */
export const head = async ({params}: {params: {hash: string | undefined}})=>{
